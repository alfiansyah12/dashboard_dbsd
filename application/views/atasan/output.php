...
<head>
  ...
  <style>
    ...
    .kpi .label{ font-size: 12px; color: var(--btn-muted); font-weight: 900; }
  ...
  <main class="app-content">
    ...
    <div class="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-3">
      ...
      <div class="d-flex gap-2">
        ...
        <a href="<?= base_url('index.php/atasan/chart'.($filter_divisi_id ? '?divisi_id='.$filter_divisi_id : '')) ?>"
    ...
    <div class="card mb-3">
      <div class="card-body py-2">
        <form class="d-flex flex-wrap align-items-center gap-2" method="get" action="<?= base_url('index.php/atasan') ?>">
          <div class="fw-bold small text-muted">Filter Division</div>

          <select name="divisi_id" class="form-select form-select-sm" style="min-width:220px;">
          ...
          <?php if($filter_divisi_id !== null && $filter_divisi_id !== ''): ?>
            ...
            <?php foreach($rows as $r): ?>
              <?php
                ...
                $badge =
                  ...
                  ($r->status === 'on going' ? 'bg-primary' :
                  ($r->status === 'terminated' ? 'bg-danger' : 'bg-secondary'));
              ?>
              <tr>
                ...
                <td style="min-width: 210px;">
                  <form method="post" action="<?= base_url('index.php/atasan/review_store') ?>" class="review-form">
                    ...
                    <div class="d-flex gap-2 mt-2">
                      ...
                      <?php if (!$isTerminated): ?>
                        <a href="<?= base_url('index.php/atasan/terminate/'.$r->pegawai_tugas_id).($filter_divisi_id ? '?divisi_id='.$filter_divisi_id : '') ?>"
          ...
          <table class="table table-hover table-sm mb-0 align-middle">
            ...
            <tbody>
              ...
              <?php if (empty($goals_rows)): ?>
                <tr><td colspan="6" class="text-center text-muted py-4">Belum ada goals.</td></tr>
              <?php endif; ?>
    ...
    <?php
      function rupiah($angka) { return number_format((int)$angka, 0, ',', '.'); }
    ?>
...
/* Charts: Target vs Realisasi & Fee/Volume */
...
new Chart(document.getElementById('trCompare'), {
  ...
  data: {
    labels,
    datasets: [
      { label: 'Target', data: targetData, borderWidth: 1 },
      { label: 'Realisasi', data: realisasiData, borderWidth: 1 }
    ]
  },
...
new Chart(document.getElementById('feeVolCompare'), {
  ...
  data: {
    labels,
    datasets: [
      { label: 'Fee Base Income', data: feeData, tension: 0.25, borderWidth: 2 },
      { label: 'Volume of Agent', data: volData, tension: 0.25, borderWidth: 2 }
    ]
  },
  options: commonOptions
});
